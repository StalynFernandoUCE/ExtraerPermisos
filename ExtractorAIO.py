...
def CSVFormatter():
    ...
    with open('data_RT.csv','w') as csv_file:
...
def Bagger(datastoredir):
    ...
    if Flag != int(0):
        ...
        CurrentApk = 0
        fieldnames=[]
        with open('data_RT.csv') as csv_file:
        ...
        for ApkName in ApkNameList:
            ...
            try:
                ...
                sys("rm -f -R " + UnpackedDir)
                print(".", end=' ')
                with open('data_RT.csv', 'a') as csv_dump:
...
def Main():
    ...
    print("EXTRACCION DE PERMISOS ANDROID (APKs CON MALWARE")
    print("Extracción de Permisos\t[*---]")
    Extract()
    sys("rm ./MalwareAPK/*.jobf")
    print("\n\nCreando Lista de Permisos Base\t[**--]")
    PermListUpdater()
    print("\n\nCreando Archivo .csv para Guardar Informacion\t[***-]")
    CSVFormatter()
    ...
    print("\n\n *************** FINALIZADO *****************  \nResultados Guardados en data_RT.csv")
